...
public class ReceiveFromZmConR {
	...
	public ResultFromSendToZM receiveResultFromZmNew(String data,String key) {
		Date date1=new Date();
		int tols=-1;//统计接收到的最大数据
		int rows=0;//统计执行的条数
		boolean isError=false;//是否错误
		...
		if (data!=null && key!=null && key.equals("sz_zm_2017-4-26")) {
			try {
				List<ZmReturnData> list=gson.fromJson(data, new TypeToken<List<ZmReturnData>>(){}.getType());
				tols=list.size();
				for (int i = 0; i < list.size(); i++) {
					ZmReturnData zm=list.get(i);
					if(!zm.getCourierNumber().trim().substring(0,2).equals("88")){
						//--------装填状态信息：是否已发货-----------------
						if (zm.getSendTime()==null) {
							zm.setCourierState("0");
	...
	//						log.warn("插入失败，开始尝试修改。失败数据："+zm);
							try {
								...
								zmReturnDataSer.update(zm,null);
								rows++;
							} catch (Exception e2) {
								e2.printStackTrace();
								log.error("【哲盟返回接口】错误：该条数据既无法插入也无法修改："+zm);
	...
	public ResultFromSendToZM receiveResultFromTPNew(String data,String key) {
		Date date1=new Date();
		int tols=-1;//统计接收到的最大数据
		int rows=0;//统计执行的条数
		int emptys=0;//除了单号其他为空的数据
			...
			try {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if(!tp.getCourierNumber().trim().substring(0, 2).equals("88")){
						...
						try {
							//--------装填其他信息-----------
							ZmReturnData zm=zmReturnDataSer.get(tp.getCourierNumber(),null);
							if (zm!=null) {
								tp.setLargeArea(zm.getLargeArea());
								tp.setSliceArea(zm.getSliceArea());
								tp.setFenbu(zm.getFenbu());
								...
								tp.setGoodsCost(zm.getGoodsCost());
								//----------开始计算是否超时---计算完之后，补上以下数据：[是否超时]----------
								zmReturnDataSer.reckon(tp);
							}
							zmReturnDataSer.add(tp,null);
							rows++;
						} catch (Exception e) {
	//					log.info("插入失败，开始尝试修改。失败数据："+tp);
							try {
								...
								//----------开始计算是否超时---计算完之后，补上以下数据：[是否超时]----------
								zmReturnDataSer.reckon(tp);
								zmReturnDataSer.update(tp,null);
								rows++;
							} catch (Exception e2) {
								e2.printStackTrace();
								log.error("【哲盟返回第三方接口】错误：该条数据既无法插入也无法修改："+tp);
